-- Split site analytics by audience (Offline / guest vs signed-in) so tool time and
-- daily visitors are not mixed when the same browser uses both modes.

-- ── Daily visitors: one row per visitor / day / audience ─────────────────────

ALTER TABLE public.site_analytics_daily_visitors
  DROP CONSTRAINT IF EXISTS site_analytics_daily_visitors_pkey;

ALTER TABLE public.site_analytics_daily_visitors
  ADD PRIMARY KEY (visitor_id, visit_date, is_guest);

-- ── Tool usage: track active seconds per audience ────────────────────────────

ALTER TABLE public.site_analytics_tool_visitor_daily
  ADD COLUMN IF NOT EXISTS is_guest boolean NOT NULL DEFAULT true;

UPDATE public.site_analytics_tool_visitor_daily AS t
SET is_guest = COALESCE(
  (
    SELECT dv.is_guest
    FROM public.site_analytics_daily_visitors AS dv
    WHERE dv.visitor_id = t.visitor_id
      AND dv.visit_date = t.visit_date
    ORDER BY dv.is_guest DESC
    LIMIT 1
  ),
  (SELECT v.is_guest FROM public.site_analytics_visitors AS v WHERE v.id = t.visitor_id),
  true
)
WHERE t.is_guest IS DISTINCT FROM COALESCE(
  (
    SELECT dv.is_guest
    FROM public.site_analytics_daily_visitors AS dv
    WHERE dv.visitor_id = t.visitor_id
...
ALTER TABLE public.site_analytics_tool_visitor_daily
  DROP CONSTRAINT IF EXISTS site_analytics_tool_visitor_daily_pkey;

ALTER TABLE public.site_analytics_tool_visitor_daily
  ADD PRIMARY KEY (visit_date, visitor_id, tool_id, sub_tool_id, is_guest);

CREATE INDEX IF NOT EXISTS site_analytics_tool_visitor_daily_date_tool_guest_idx
  ON public.site_analytics_tool_visitor_daily (visit_date DESC, tool_id, sub_tool_id, is_guest);

-- ── Ingest: write guest and signed-in buckets separately ─────────────────────
...
-- ── Dashboard: audience-split summary ────────────────────────────────────────
