...
-- Share-weighted payout from ore profit actual total (÷ total shares × member shares).
...
BEGIN
  ...
  v_total_payout := public.mining_ledger_ore_profit_actual_total(p_data);
  IF v_total_payout IS NULL OR v_total_payout <= 0 THEN
    RETURN 0;
  END IF;
