...
> Discord supplies `global_name` as the display name. The existing `handle_new_user` trigger maps provider metadata automatically.
...
npx supabase functions deploy record-analytics-ping
```
